...
def create_epochs_evoked_and_behavioral_metadata(
    ...
    *,
    ...
    derivatives = [
        ...
        derivatives_folder / f"{fname_stem}_step60b-1-originalmast-epo.fif",  
        derivatives_folder / "plots"/ f"{fname_stem}_step60b-1-originalmast_stimlocked-epochs-rejected.svg",  
        derivatives_folder / f"{fname_stem}_step60b-1-originalmast_stimlocked-cleaned-epo-drop-log.csv",  

        derivatives_folder / f"{fname_stem}_step60b-1_stimlocked_originalmast-standard-ave.fif",  
        derivatives_folder / f"{fname_stem}_step60b-1_stimlocked_originalmast-target-ave.fif",  
        derivatives_folder / f"{fname_stem}_step60b-1_stimlocked_originalmast-novel-ave.fif",  
    ...
    try:
        ...
        with open(
            ...
            for condition in ["standard", "target", "novel"]:
                total = count_stim_before.get(events_mapping[condition], 0)
                remaining = count_stim_after.get(events_mapping[condition], 0)
                bad = drop_reasons[condition].get("bad_segment", 0)
                ptp = drop_reasons[condition].get("ptp", 0)
...
#####,
        ...
        with open(
            ...
            for condition in ["response"]:
                ...
                bad = drop_reasons_response_2000resp[condition].get("bad_segment", 0)
                ptp = drop_reasons_response_2000resp[condition].get("ptp", 0)
 ...
 """
        epochs.save(
            derivatives_folder / f"{fname_stem}_step60b-1_stimlocked_originalmast-epo.fif",
        )

        for cond in epochs.event_id:
            evokeds[cond].save(
                derivatives_folder / f"{fname_stem}_step60b-1_stimlocked_originalmast-{cond}-ave.fif"
            )
...
#####,
        ...
        fig_drops.savefig(
            derivatives_folder
            / "plots"
            / f"{fname_stem}_step42_stimlocked-epochs-rejected.svg", #8
            transparent=True,
        )
        #for cond in epochs.event_id:
        #    evokeds[cond].save(
        ...
        with open(
            derivatives_folder
            / f"{fname_stem}_step42_stimlocked-cleaned-epo-drop-log.csv",#8
            "w",
        ) as file:
            file.write(
...
def _create_epochs_evoked_and_behavioral_metadata(
    ...
    if "response" in events_id:
        ...
        (
            ...
            epochs_response_2000resp, events_response, response=True
        )
 ...
 """
    ...
    return (
        ...
        ##drop_reasons_response_2000resp,
        ##fig_drops_response_2000resp,
        ##None if epochs_response_2000resp is None else epochs_response_2000resp.average(), #evokeds_response_2000resp,
        # epochs_response_2000resp_freq,
...
def plot_drop_bar(drop_reasons: dict[str, dict[str, int]], count_stim_before: Counter) -> plt.Figure:
    ...
    sns.barplot(data=df, x="Condition", y="Count", hue="Reason", ax=ax)
...
def _drop_bad_epochs(
    ...
    if epochs.metadata is not None and "response_correct" in epochs.metadata.columns:
        incorrect_mask = epochs.metadata["response_correct"] == False
        incorrect_indices = np.where(incorrect_mask)[0]
        epochs.drop(incorrect_indices, reason="incorrect_trial")
    else:
        incorrect_indices = []
    ...
    #         novel=dict(bad_segment=0, ptp=0, too_short=0, after_response=0),
    #     )
    for i, (ev, drops) in enumerate(zip(events, epochs.drop_log)):
        if len(drops) == 0:
            continue
        event_type = events_mapping[ev[2]]
        ...
        if all(elt in epochs.ch_names for elt in drops):
            ...
            drop_reasons[event_type]["too_short"] += 1
        elif any(elt == "incorrect_trial" for elt in drops):
            drop_reasons[event_type]["incorrect_trial"] += 1
        else:
            raise ValueError(f"Unknown drop reason: {drops}")
    ...
    #    event_type = events_mapping.get(event_code, "response" if response else "unknown")
    #    drop_reasons[event_type]["incorrect_trial"] += 1
    #    #for ev_type in ["standard", "target", "novel"]:
    #    #    drop_reasons[ev_type]["incorrect_trial"] = len(epochs[ev_type]["response_correct == False"])

    print(drop_reasons)
    fig = epochs.plot_drop_log()
    fig = plot_drop_bar(drop_reasons, count_stim_before)
    count_stim_after = Counter(epochs.events[:, 2])
    return epochs, count_stim_before, count_stim_after, drop_reasons, fig
