...
def create_epochs_evoked_and_behavioral_metadata(
    ...
    *,
    ...
    derivatives = [
        ...
        derivatives_folder / f"{fname_stem}_step31zzc_stimlocked-EOGALL-epo.fif",
    ...
    try:
        ...
        raw_wEOG = read_raw_fif(
            derivatives_folder
            / f"{fname_stem}_step30zz_preprocessed_EOGEEGall_raw.fif",
            preload=True,
        )
        ...
        (
            ...
            # drop_reasons_response_2000resp,
            # fig_drops_response_2000resp
        ) = _create_epochs_evoked_and_behavioral_metadata(raw_wEOG)
...
#####,
        ...
        for cond in epochs_wEOG.event_id:
            evokeds_wEOG[cond].save(
                derivatives_folder
                / f"{fname_stem}_step31zzc_stimlocked-EOGALL-{cond}-ave.fif"
            )
...
def _create_epochs_evoked_and_behavioral_metadata(
 ...
 """
    ...
    (
        ...
        _,
        _,
    ) = _drop_bad_epochs(epochs_wEOG, events, response=False)
    ...
    if metadata is None:
        evokeds_wEOG = dict(
            (cond, epochs_wEOG[cond].average(picks="all"))
            for cond in epochs_wEOG.event_id
        )
    else:
        evokeds_wEOG = dict(
...
def _drop_bad_epochs(
    ...
    if response:
        drop_reasons = dict(response=dict(bad_segment=0, ptp=0, incorrect_trial=0))
    else:
        drop_reasons = {
            etype: dict(bad_segment=0, ptp=0, after_response=0, incorrect_trial=0)
    ...
    for idx in incorrect_indices:
        ...
        event_type = events_mapping.get(
            event_code, "response" if response else "unknown"
        )
        drop_reasons[event_type]["incorrect_trial"] += 1
