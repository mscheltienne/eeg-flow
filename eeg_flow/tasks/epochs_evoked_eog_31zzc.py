...
#####,
        ...
        epochs_wEOG.save(derivatives_folder / f"{fname_stem}_step31zzc_stimlocked-EOGALL-epo.fif")
        ...
        for cond in epochs_wEOG.event_id:
            evokeds_wEOG[cond].save(
                derivatives_folder / f"{fname_stem}_step31zzc_stimlocked-EOGALL-{cond}-ave.fif"
            )
...
def _drop_bad_epochs(
    ...
    if response:
        drop_reasons = dict(response=dict(
            bad_segment=0, ptp=0, incorrect_trial=0
        ))
    else:
        drop_reasons = {
            etype: dict(
                bad_segment=0, ptp=0, after_response=0, incorrect_trial=0
